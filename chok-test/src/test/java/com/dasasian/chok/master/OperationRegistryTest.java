...
public class OperationRegistryTest extends AbstractMasterNodeZkTest {
    ...
    private final MasterOperation masterOperation = Mockito.mock(MasterOperation.class);
    ...
    public void testAllOperationsDoneWithoutResults() throws Exception {
        ...
        Mockito.verify(masterOperation, Mockito.times(1)).nodeOperationsComplete(masterContext, operationResults);
    }
    ...
    public void testAllOperationsDone() throws Exception {
        ...
        Mockito.verify(masterOperation, Mockito.times(1)).nodeOperationsComplete(eq(masterContext), argument.capture());
    ...
    private OperationWatchdog beginMasterOperation(List<Node> nodes, MasterOperation operation) {
        ...
        for (Node node : nodes) {
            OperationId operationId = getInteractionProtocol().addNodeOperation(node.getName(), Mockito.mock(NodeOperation.class));
