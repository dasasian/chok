...
public class MasterQueueTest extends AbstractZkTest {
    ...
    public void testKeepStateBetweenExecutionAndCompletion() throws Exception {
        ...
        masterOperation.execute(Mockito.mock(MasterContext.class), Collections.EMPTY_LIST);
        ...
        masterOperation.nodeOperationsComplete(Mockito.mock(MasterContext.class), Collections.EMPTY_LIST);
    }
    ...
    public void testWatchDogCleanup() throws Exception {
        ...
        Mockito.doThrow(new IllegalStateException("test exception")).when(zkClientSpy).delete(Matchers.endsWith(elementName));
        try {
            queue.moveOperationToWatching(masterOperation, operationIds);
            Mockito.verify(zkClientSpy).delete(Matchers.endsWith(elementName));
