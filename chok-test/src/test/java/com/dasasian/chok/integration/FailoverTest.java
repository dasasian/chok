...
public class FailoverTest extends AbstractTest {
    ...
    @Rule
    public TestLoggerWatcher nodeProxyManagerLoggingRule = TestLoggerWatcher.logErrors(NodeProxyManager.class, "testNodeFailure");

    @Rule
    public TestLoggerWatcher nodeInteractionLoggingRule = TestLoggerWatcher.logErrors(NodeInteraction.class, "testNodeFailure");
    ...
    public void testNodeFailure() throws Exception {
        miniCluster.deployIndex(testIndex, 2);

        try (final SimpleTestClient client = new SimpleTestClient(miniCluster.getZkConfiguration())) {
            assertEquals("[shard0:1, shard1:1, shard2:1]", client.testRequestFormattedResult("1", null));
