...
public class MapFileServerTest extends AbstractTest {
    ...
    public void testShardA1() throws Exception {
        ...
        server.addShard(SHARD_A_1, MapFileTestResources.MAP_FILE_A.toPath().resolve("a1"));
        ...
        assertEquals("427", server.getShardMetaData(SHARD_A_1).get(IContentServer.SHARD_DISK_USAGE_KEY));
    ...
    public void testShardA2() throws Exception {
        MapFileServer server = new MapFileServer();
        server.init(NODE_NAME, nodeConfigurationFactory.getConfiguration());
        server.addShard(SHARD_A_2, MapFileTestResources.MAP_FILE_A.toPath().resolve("a2"));
        assertEquals("3", server.getShardMetaData(SHARD_A_2).get(IContentServer.SHARD_SIZE_KEY));
        assertEquals("425", server.getShardMetaData(SHARD_A_2).get(IContentServer.SHARD_DISK_USAGE_KEY));
    ...
    public void testMapFile1() throws Exception {
        ...
        server.addShard(SHARD_A_1, MapFileTestResources.MAP_FILE_A.toPath().resolve("a1"));
        server.addShard(SHARD_A_2, MapFileTestResources.MAP_FILE_A.toPath().resolve("a2"));
        server.addShard(SHARD_A_3, MapFileTestResources.MAP_FILE_A.toPath().resolve("a3"));
        server.addShard(SHARD_A_4, MapFileTestResources.MAP_FILE_A.toPath().resolve("a4"));
        assertEquals("3", server.getShardMetaData(SHARD_A_1).get(IContentServer.SHARD_SIZE_KEY));
        assertEquals("3", server.getShardMetaData(SHARD_A_2).get(IContentServer.SHARD_SIZE_KEY));
        ...
        assertEquals("427", server.getShardMetaData(SHARD_A_1).get(IContentServer.SHARD_DISK_USAGE_KEY));
        assertEquals("425", server.getShardMetaData(SHARD_A_2).get(IContentServer.SHARD_DISK_USAGE_KEY));
        assertEquals("398", server.getShardMetaData(SHARD_A_3).get(IContentServer.SHARD_DISK_USAGE_KEY));
        assertEquals("453", server.getShardMetaData(SHARD_A_4).get(IContentServer.SHARD_DISK_USAGE_KEY));
    ...
    public void testBothMapFiles() throws Exception {
        ...
        server.addShard(SHARD_A_3, MapFileTestResources.MAP_FILE_A.toPath().resolve("a3"));
        server.addShard(SHARD_A_4, MapFileTestResources.MAP_FILE_A.toPath().resolve("a4"));
        server.addShard(SHARD_B_1, MapFileTestResources.MAP_FILE_B.toPath().resolve("b1"));
        server.addShard(SHARD_B_2, MapFileTestResources.MAP_FILE_B.toPath().resolve("b2"));
        assertEquals("3", server.getShardMetaData(SHARD_A_1).get(IContentServer.SHARD_SIZE_KEY));
        assertEquals("3", server.getShardMetaData(SHARD_A_2).get(IContentServer.SHARD_SIZE_KEY));
        assertEquals("2", server.getShardMetaData(SHARD_A_3).get(IContentServer.SHARD_SIZE_KEY));
        assertEquals("4", server.getShardMetaData(SHARD_A_4).get(IContentServer.SHARD_SIZE_KEY));
        ...
        assertEquals("398", server.getShardMetaData(SHARD_A_3).get(IContentServer.SHARD_DISK_USAGE_KEY));
        assertEquals("453", server.getShardMetaData(SHARD_A_4).get(IContentServer.SHARD_DISK_USAGE_KEY));
        assertEquals("426", server.getShardMetaData(SHARD_B_1).get(IContentServer.SHARD_DISK_USAGE_KEY));
        assertEquals("425", server.getShardMetaData(SHARD_B_2).get(IContentServer.SHARD_DISK_USAGE_KEY));
