...
public class ShardDeployOperationTest extends AbstractNodeOperationMockTest {
    ...
    public void testDeploy() throws Exception {
        ...
        when(shardManager.installShard((String) notNull(), (String) notNull())).thenReturn(shardFolder);
        when(contentServer.getShardMetaData((String) notNull())).thenReturn(shardMD);
    ...
    public void testDeployWithOneFailingShard() throws Exception {
        ...
        doThrow(new Exception("testException")).when(contentServer).addShard(failingShard, shardFolder);
        ...
        verify(shardManager).uninstallShard(eq(failingShard));
    }
}
