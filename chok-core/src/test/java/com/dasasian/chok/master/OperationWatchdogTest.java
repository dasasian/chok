...
public class OperationWatchdogTest extends AbstractTest {
    ...
    private MasterOperation masterOperation = mock(MasterOperation.class);
    ...
    private MasterQueue masterQueue = mock(MasterQueue.class);
    ...
    public void testWatchdogCompletion_OperationsDone() throws Exception {
        ...
        when(protocol.getLiveNodes()).thenReturn(Arrays.asList(operationIds.get(0).getNodeName()));
        when(protocol.isNodeOperationQueued(operationIds.get(0))).thenReturn(true);
        ...
        verify(masterQueue).removeWatchdog(watchdog);
    }
    ...
    public void testWatchdogCompletion_NodeDown() throws Exception {
        ...
        // node down
        when(protocol.getLiveNodes()).thenReturn(Collections.EMPTY_LIST);
        watchdog.checkDeploymentForCompletion();
        assertTrue(watchdog.isDone());
        verify(masterQueue).removeWatchdog(watchdog);
    }
