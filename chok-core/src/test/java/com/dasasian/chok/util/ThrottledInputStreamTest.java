...
public class ThrottledInputStreamTest extends AbstractTest {
    ...
    public void testThrottleReadFromMultipleInputStreams() throws Exception {
        ...
        assertThat((readThread1.getReadBytes() + readThread2.getReadBytes()) / (expectedReadTime / 1000), almostEquals(bytesPerSecond, 700));
    }

    private ReadThread checkThrottledRead(long expectedReadTime, int bytesPerSecond, boolean readWithBuffer) throws InterruptedException {
        ThrottledInputStream throttledInputStream = new ThrottledInputStream(new EndlessFastInputStream(), new ThrottleSemaphore(bytesPerSecond));
