...
public class ShardManager {
        ...
        for (int i = 0; i < maxTries; i++) {
            ...
            try {
                ...
                try {
                    ...
                    if (isZip) {
                        FileUtil.unzip(path, shardTmpFolder, fileSystem, System.getProperty("chok.spool.zip.shards", "false").equalsIgnoreCase("true"));
                    }
                    else {
    ...
    private static class ThrottledFileSystem extends FileSystem {
        ...
        public FSDataInputStream open(Path path, int bufferSize) throws IOException {
            ThrottledInputStream throttledInputStream = new ThrottledInputStream(_fileSystemDelegate.open(path, bufferSize), _throttleSemaphore);
