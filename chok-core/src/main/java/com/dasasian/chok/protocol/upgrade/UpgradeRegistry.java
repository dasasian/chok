...
public class UpgradeRegistry {
    ...
    public static UpgradeAction findUpgradeAction(InteractionProtocol protocol, Version distributionVersion) {
        ...
        if (clusterVersion == null) {
            ...
            boolean isPre0_6Cluster = protocol.getZkClient().exists(protocol.getZkConfiguration().getRootPath() + "/indexes");
            if (isPre0_6Cluster) {
        ...
        for (VersionPair versionPair : _upgradeActionsByVersion.keySet()) {
            ...
            if (currentVersionPair.getFromVersion().startsWith(versionPair.getFromVersion()) && currentVersionPair.getToVersion().startsWith(versionPair.getToVersion())) {
